...
import {IsString, IsNotEmpty, IsEmail, IsOptional, Matches} from "class-validator";// các decorator để validate dữ liệu đầu vào như @IsEmail để kiểm tra xem trường đó có phải là email hay không, @IsString để kiểm tra xem trường đó có phải là chuỗi hay không, @IsOptional để chỉ định rằng trường này là tùy chọn, @IsNumber để kiểm tra xem trường đó có phải là số hay không
...
export class UpdateUserByAdmin{
    ...
    // thêm remaining_device_quota
    @Expose()
    @IsOptional()
    @Matches(/^[0-9]+$/, {message: 'Remaining device quota must be a number'})
    new_remaining_device_quota?: number;
